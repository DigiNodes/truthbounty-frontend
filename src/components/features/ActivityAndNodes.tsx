...
interface ActivityAndNodesProps {
   ...
   *  empty state until real data is available (V2-FE-016: fixtures live in
   *  tests/Storybook only). */
  data?: ActivityDatum[];
  isLoading?: boolean;
}

const ActivityAndNodes = ({ data = [], isLoading = false }: ActivityAndNodesProps) => {
  if (isLoading) {
      ...
      <div className="flex-1 w-full h-full min-h-0">
        {data.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-center">
            <p className="text-sm text-[#a1a1aa]">No activity data available yet</p>
            <p className="text-xs text-[#71717a]">
              Activity will appear here once the indexer is connected.
            </p>
          </div>
        ) : (
