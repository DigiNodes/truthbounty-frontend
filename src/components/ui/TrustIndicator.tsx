...
export default function TrustIndicator() {
  ...
  const display = reputation !== null ? reputation : "—";

  return (
    <div className="flex items-center space-x-1" aria-label={`Trust score: ${display}`}>
      <div className={`${color} w-3 h-3 rounded-full`} aria-hidden="true" />
      <span className="text-xs text-[#a1a1aa]">trust: {display}</span>
    </div>
  );
}
